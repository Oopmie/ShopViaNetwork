...
fun CreatePass(navController: NavController, viewModel: AuthViewModel = koinViewModel(), onSuccess: () -> Unit) {
    Column( modifier = Modifier.fillMaxSize().padding(20.dp, 60.dp)) {
        ...
        Column( modifier = Modifier.padding(vertical = 60.dp).fillMaxHeight(0.57f),
            ...
            PassTextField(value = pass1, onValueChange = {
                    pass1 = it
            ...
            Button(modifier = Modifier.fillMaxWidth().height(56.dp),
                onClick = { pass1Error = pass1.isBlank()
                    pass2Error = pass2.isBlank() || pass1 != pass2
                    val isSecure = viewModel.isPasswordValid(pass1)
                    if (pass1Error || pass2Error) {
                    } else if (!isSecure) { pass1Error = true }
                    else { viewModel.registerAndSaveAll(pass1) {
                        onSuccess() } } },
                ...
                shape = RoundedCornerShape(10.dp) ) { Text("Далее") }
        }
    }
}